...
void CACHE::handle_writeback()
{
    while (writes_available_this_cycle > 0)
    {
        ...
        if (way < NUM_WAY) // HIT
        {
            ...
            if ((cache_type == IS_L1I) && (handle_pkt.conf > 0)) {
                l1i_update_replacement_state_conf(handle_pkt.cpu, set, way, fill_block.full_addr, handle_pkt.ip, 0, handle_pkt.type, 1, handle_pkt.conf);
            }
            else {
...
void CACHE::readlike_hit(std::size_t set, std::size_t way, PACKET &handle_pkt)
{
    ...
    if ((cache_type == IS_L1I) && (!(handle_pkt.conf)) && handle_pkt.type == PREFETCH) {
        l1i_update_replacement_state_conf(handle_pkt.cpu, set, way, hit_block.full_addr, handle_pkt.ip, 0, handle_pkt.type, 1, handle_pkt.conf);
    }
    else {
...
bool CACHE::filllike_miss(std::size_t set, std::size_t way, PACKET &handle_pkt)
{
    ...
    if ((cache_type == IS_L1I) && (handle_pkt.conf > 0)) {
        l1i_update_replacement_state_conf(handle_pkt.cpu, set, way, handle_pkt.full_addr, handle_pkt.ip, 0, handle_pkt.type, 0, handle_pkt.conf);
    }
    else {
