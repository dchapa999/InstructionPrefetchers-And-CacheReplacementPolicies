...
//   for (int n = 0; n < m_numHashes; n++) 
//   {
//       if ((m_bits[nthHash(n, hashValues[0], hashValues[1], m_bits.size())]) == 0) 
//       {
//           return false;
//       }
//   }
...
//
#define SAMPLE_COUNT 128 // L1I_WAY<<3 
#define OV_size 128
int ALIAS_TAB_SIZE;

uint32_t rrpv[8192][L1I_WAY];
...
void CACHE::l1i_initialize_replacement()
{
    ...
    int sample_sets_num = 20*NUM_CPUS;
...
{
    uint64_t top8 = (tag_addr<<(6+L1I_SET_LOG2)) >> 56;
...
/*
        ...
        if(set == 7494)
            cout << "cfg 3/4" << endl;
*/
        uint64_t tag_addr = full_addr >> (6 + L1I_SET_LOG2);
            // ---- Update History
